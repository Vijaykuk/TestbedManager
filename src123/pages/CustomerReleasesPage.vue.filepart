...
ganttOptions1:{
  ...
  watch:{
    showExecutionPie:function(newVal){
      if(!newVal){
        this.totalActualQuality=this.totalQuality;
      }
      else{
        this.totalActualQuality=this.totalProjectedQuality;
      }
    }
  },
  computed: {
    ...
    cdetsTableValues(){
      let arr=[],obj={};
      for(let i=0;i<this.selectedCdets.length;i++){
        obj={};
        obj["Identifier"]=this.selectedCdets[i].defect_id;
        obj["Headline"]=this.selectedCdets[i].headline;
        obj["Component"]=this.selectedCdets[i].component;
        obj["Submitter"]=this.selectedCdets[i].submitter;
        obj["Status"]=this.selectedCdets[i].status;
        obj["#TCs"]=this.selectedCdets[i].tcCount;
        arr.push(obj);
      }
      return arr;
    }
 ...
 /*   drawXAxisBackground(e){
   ...
   var position1 = {
    ...
    var position2 = {
     ...
     if (!xAxis1.background) {
    xAxis1.background = chart.renderer.rect().attr({
      fill: 'pink',
      zIndex: 0
    }).attr(position1).add();
  } else {
    xAxis1.background.animate(position1);
  }
 /* if (!xAxis2.background) {
    xAxis2.background = chart.renderer.rect().attr({
      fill: 'yellow',
      zIndex: 0
    }).attr(position2).add();
  } else {
    xAxis2.background.animate(position2);
  }
       }
    },*/
  ...
  var chart = e.target,
    ...
    title.background=chart.renderer.rect().attr({
      fill: 'pink',
      zIndex: 0
    }).add();
  ...
  if (!xAxis1.background) {
    ...
    xAxis1.background.animate(position);
  }
 /* if (!xAxis2.background) {
    xAxis2.background = chart.renderer.rect().attr({
      fill: 'pink',
    ...
    xAxis2.background.animate(position);
  }
},*/
    loadCFDS() {
      this.loading = false;
    ...
    releaseBoxClicked(tempName){
      let relId=this.getIdForRelease(tempName);

       this.selectedRelease = relId;
        this.drawChart(relId,"Release");
         this.releaseFlag="release";
                this.loadWeeklyReport(relId);
                this.loadGanttChart(relId);
               this.selectedReleaseName = tempName;
                 this.ganttOptions.title.text="Project Plan - <span style='font-size:15px'>"+ this.tempName+"</span>"
                this.ganttOptions1.title.text="Project Plan - <span style='font-size:15px'>"+ this.tempName+"</span>"
               
                event.preventDefault();
                event.stopPropagation();
    },
    custBoxClicked(custName){
      ...
      for(let l=0;l<custlist.length;l++){
        if(custlist[l].customer_name===custName){
          filteredCustList.push(custlist[l]);
        }
      }
      self.summary_customers=filteredCustList;

      let data=self.testcases;
      let filteredData=[];
       for(let m=0;m<data.length;m++){
         if(data[m].tc_customer===custName){
       ...
       self.summaryObj.defectSummary=self.getRelDefectSummary(custName);
             self.testcases=filteredData;
             self.selectedTestcases=filteredData;
             self.showingUnique=false;
             self.testcasesFlag+=1;
             self.updateCdetsDetails();
            self.totalTestcases =  self.testcases.length;
           // self.loadTopSummaryData(testcases);
            self.loadSummaryData(self.testcases);

     let  weeklyData=self.weeklyReport;
    ...
    /*    testcaseService.loadUniqueTestcasesGraphs(self.releaseFlag,id).then(
          (data) => {
            //graphs
          },
       (error)=>{
         self.loading = false;
            ...
            self.showErrorModal = true;
            self.modalErrorMessage =
              "Error while loading unique testcase, Please try again later";
       });*/
   ...
   /* responsive: {
    ...
    loadSummaryData(testcases) {
      ...
      for (let i = 0; i < testcases.length; i++) {
        ...
        testcases[i].tc_platform=testcases[i].tc_platform===""?null:testcases[i].tc_platform;
        testcases[i].tc_testExecOwner=testcases[i].tc_testExecOwner===""?null:testcases[i].tc_testExecOwner;
        testcases[i].tc_source=testcases[i].tc_source===""?null:testcases[i].tc_source;
       ...
       testcaseRoles.push(testcases[i].tc_role);
        ...
        if(testcases[i].automation_flag &&
          testcases[i].automation_flag === true){
            totalAutomated+=1;
          }
        if (
          (
         ...
         if (
            testcases[i].tc_procedure === null ||
            testcases[i].tc_procedure === undefined ||
            testcases[i].tc_procedure === "" 
        ) {
          testcasesWithoutProcedure += 1;
        }
      }
      ...
      this.totalAutomated=parseInt(totalAutomated/this.totalTestcases * 100); 
      this.totalAutomatedCount=totalAutomated;
      ...
      this.totalExecutedCount=testcaseExecuted;
      this.totalPending=parseInt(testcasePending/this.totalTestcases * 100);
      this.totalExecuted=parseInt(testcaseExecuted/this.totalTestcases * 100);


      
      this.ganttOptions.series[0].data[2].completed.amount=this.totalExecuted/100;
      this.ganttOptions.series[0].data[2].completed.fill="green";
     
      this.ganttOptions1.series[0].data[0].completed.amount=this.totalExecuted/100;
      this.ganttOptions1.series[0].data[0].completed.color="green";

      debugger;
    
     this.totalQuality=parseInt(testcasePassed/testcaseExecuted*100);
     this.totalActualQuality=this.totalQuality;
     
      this.totalRegression = testcasesRegression;
      this.testcasesWithLogs = testcasesWithLogs;

      this.testcasesWithoutProcedure=testcasesWithoutProcedure;
     ...
     let defectsSummaryDetails = {};
      ...
      for (let kg = 0; kg < testcases.length; kg++) {
        ...
        for (let s = 0; s < temp123.length; s++) {
          ...
          if(defectsSummaryDetails[temp123[s]]){
    ...
    loadMultiBarChartDataForUser(testcases,allUsers){
     ...
     for(let ra=0;ra<this.cdets.length;ra++){
      ...
      let testcaseList=[],defectList=[],passedList=[],failedList=[],pendingList=[];
        ...
        let total=[],mhHave=[],cvtFound=[],cvtObserved=[],planned=[],actual=[],passed=[],failed=[],cvtTotal=[],executed=[],pending=[];
          ...
          let categoryObj={};
            ...
            for(let k=0;k<categorynames.length;k++){
              ...
              valueObj.push(i);
              valueObj.push(k);
              valueObj.push(categoryObj[categorynames[k]]);
    ...
    // heatMapOptions.series[0].dataLabels={enabled:true};
/*
     let userQualityHeatMapOptions=this.loadHeatMapOptions();
     userQualityHeatMapOptions.xAxis.categories=weekDates;
     userQualityHeatMapOptions.yAxis.categories=this.allUsers;

     let userDefectsHeatMapOptions=this.loadHeatMapOptions();
     userDefectsHeatMapOptions.xAxis.categories=weekDates;
     userDefectsHeatMapOptions.yAxis.categories=this.allUsers;


     for(let m=0;m<this.allUsers.length;m++){
    ...
    updateAhaTableRow(){
      let self=this;
      testcaseService.updateAhaReportByReleaseId(this.ahaList,self.selectedRelId).then(
          (data) => {
   ...
   updateExecutionDefectSummary(type){
      ...
      testcaseService.updateSummary(this.summaryObj,this.selectedRelId).then(
        (data) => {
           this.loading = false;
              this.loadingMessage = "";
...
if(this.eventObj.name=="" || this.eventObj.name===undefined || this.eventObj.name===null||
    ...
    showSummaryDetails(obj){
      this.summaryDetailsObj=obj;
  ...
  <div class="customerReleaseDiv" id="customerReleaseDiv">
    ...
    <div class="dashboardDiv">
     ...
     <!-- <div class="summaryBox" :key="index">
      ...
      <div class="summaryBox" :key="index">
         <div :class="'custBox cust_box'+index%7"  style="pointer:cursor;" @click="custBoxClicked(summary_obj.name)">
             <div class="row custSummeryTitleDiv">
     ...
     <div class="secondPage secondPageLight" v-show="showCustGraphs">
      ...
      <div class="defectTableTitle">
       ...
       <div class="row defectTablecontainer showRelContainer" v-if="showReleaseSummary" style="margin-top:-20px;">
        <div class="saveButtonContainer">
         <button
            class="btn btn--primary tableGrphBtn"
              @click="updateExecutionDefectSummary()"
              >
           <span 
              class="icon-save icon-small clickable tableIcons"
            ></span>
      ...
      <vue-editor id="rel_summary"  class="proceduretextarea rel_summary" v-model="summaryObj.summary"  @enter="autoResize"
              @change="autoResize"
      ...
      <div class="defectTableTitle">
        Issues <span
          v-if="!showReleaseDefectSummary"
          class="icon-chevron-right interaction"
          style="margin-top:-3p;margin-left: 10px;"
          v-on:click="showReleaseDefectSummary=!showReleaseDefectSummary"
         ></span>
         <span
          v-if="showReleaseDefectSummary"
          class="icon-chevron-down interaction"
       ...
       <div class="row defectTablecontainer showRelContainer" v-if="showReleaseDefectSummary" style="margin-top:-20px;">
        <div class="saveButtonContainer">
          <button
            class="btn btn--primary tableGrphBtn"
              @click="updateExecutionDefectSummary()"
          ...
          <vue-editor id="rel_defect_summary"  class="proceduretextarea rel_summary" v-model="summaryObj.defectSummary"  @enter="autoResize"
              @change="autoResize"
              @load="autoResize"></vue-editor>

          <!--<textarea
              v-model="rel_defect_summary"
              style="display: block;"
              class="proceduretextarea rel_summary"
              @enter="autoResize"
              @change="autoResize"
      ...
      <div class="defectTableTitle">
       ...
       <div class="" v-show="showAhaTable">
        ...
        <table id="ahaTable" class="table table--highlight secondaryTable">
            ...
            <tr v-if="!editingAha"  :key="index">
            <td class="ahaTableTd">{{aObj.featureId}}</td>
             ...
             <td>
             <input type="text" class="weeklyTableText" v-model="aObj.enablement_date" >
             </td>
             <td>
      ...
      <!-- <vue-bootstrap-table
            id="cdetsTable"
            :columns="cdetsTableColumns"
            :values="cdetsTableValues"
            :show-filter="true"
      ...
      <div class="slidesBtn">
          ...
          <div id="showTotalExecuted">
           ...
           <button class="r_summaryDiv c_qualitySummary">
                <div class="totalCount">{{ summaryButtonsDetails.resultQuality }}%</div>
                <div class="summaryname">Quality</div>
           </button>
              <button
                class="r_summaryDiv c_platforms"
                @click="showSummaryDetails(summaryButtonsDetails.platform)"
              >
               ...
               <button
                class="r_summaryDiv c_roles"
                @click="showSummaryDetails(summaryButtonsDetails.role)"
              >
               ...
               <button
                class="r_summaryDiv c_features"
                @click="showSummaryDetails(summaryButtonsDetails.feature)"
              >
