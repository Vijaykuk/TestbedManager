...
export default {
    name: "ExecutionSummaryPageHome",
    props: ["customerName", "releaseId", "releaseFlag", "folderName","weeklyReport","currentWeekData","mhDefectsCount","mainCdets"],
     data() {
    return {
      weekIndex:0,
      fcsRDetails:[
         { "cvtType":"Hardware Covered",
          "status":5,
          "details":[
              "NCS5500: 5508, 55A1, 5501, 36p100G, 18H18F"
          ]
         }
      ],
    ...
    handler(mainCdets) {
      ...
      this.mainCdetsArray={};
        ...
        for(let f=0;f<mainCdets.length;f++){
           if((mainCdets[f].status==='A' || mainCdets[f].status==='I' || mainCdets[f].status==='N' || mainCdets[f].status==='O')&&
        (mainCdets[f].attribute && (mainCdets[f].attribute.indexOf(Str1)>-1 || mainCdets[f].attribute && mainCdets[f].attribute.indexOf(Str2)>-1)))
          {
        ...
        testcaseService.loadExecDetails(self.releaseFlag,id).then(
          (data) => {
           ...
           if(self.releaseFlag==="release"){
             ...
             self.custflag=true;
              let obj={},fname="",tmp={},temp={},tmpProject={},folders=[];
              let data1={};
              self.execProjects={};
              self.execCustomer={};
              self.execFolders={};
      ...
      loadExecTable(){
        ...
        for(let i=0;i<data.length;i++){
              categoryObj={};
               ...
               let executed=data[i].executed!=null?parseInt(data[i].executed):0;
               ...
               let total=data[i].total_testcases!=null?parseInt(data[i].total_testcases):0;
               if(i===6){
               ...
               categoryObj["pending"]= total>0?(pending/total*100).toFixed(2):0;
               categoryObj["planned"]= total>0?(planned/total*100).toFixed(2):0;
               categoryObj["executed"]=total>0?(executed/total*100).toFixed(2):0;
               categoryObj["passed"]=total>0?(passed/total*100).toFixed(2):0;
               categoryObj["failed"]=total>0?(failed/total*100).toFixed(2):0;
               ...
               categoryObj["quality"]=data[i].quality_percentage!=null?data[i].quality_percentage.toFixed(2):0;
               categoryObj["projectedQuality"]=data[i].projected_percentage!=null?data[i].projected_percentage.toFixed(2):0;
               ...
               categoryObj["pending_enablement_total"]=data[i].pending_enablement_total?data[i].pending_enablement_total:0;
         ...
         for(let gt=0;gt<data.length;gt++){
           if(new Date(data[gt].end_date)>=new Date() && new Date(data[gt].start_date)<new Date()){
             if(!pendingFlag){
         ...
         for(let g=0;g<data.length;g++){
           if(new Date(data[g].end_date)>new Date()){
             if(!pendingFlag1){
              ...
              categoryObj={};
               let planned=data[g].planned!=null?parseInt(data[g].planned):0;
               let total=data[g].total_testcases!=null?parseInt(data[g].total_testcases):0;
               categoryObj["weekDates"]=data[g].end_date.substring(0,10);
               categoryObj["pending"]=data[g].pending?data[g].pending:"";
               categoryObj["planned"]= total>0?(planned/total*100).toFixed(2):"";
               ...
               categoryObj["plannedCount"]=planned;
               categoryObj["executedCount"]=data[g].executed?data[g].executed:"";
               categoryObj["passedCount"]=data[g].passed?data[g].passed:"";
               categoryObj["failedCount"]=data[g].failed?data[g].failed:"";
               categoryObj["pendingCount"]=data[g].pending?data[g].pending:"";
                categoryObj["quality"]=data[g].quality_percentage!=null?data[g].quality_percentage.toFixed(2):0;
               categoryObj["projectedQuality"]=data[g].projected_percentage!=null?data[g].projected_percentage.toFixed(2):0;
                  categoryObj["pendingEnablement"]=0;
               let observed=data[g].defects_observed!=null?data[g].defects_observed:0;
               let found=data[g].defects_found_CVT!=null?data[g].defects_found_CVT:0;

               categoryObj["cvtTotal"]=
               categoryObj["cvtFound"]=data[g].cvt_raised_found;
               categoryObj["cvtObserved"]=observed;
               categoryObj["mhHave"]=data[g].open_mh?data[g].open_mh:"";
               categoryObj["pending_enablement_total"]=data[g].pending_enablement_total?data[g].pending_enablement_total:0;
      ...
      return ret;
          ...
          for(let k=0;k<this.tableArray.length;k++){
            if(weekNum===this.tableArray[k].weekNumber){
              ...
              document.getElementById('selectedWeekHighlights').innerHTML=this.tableArray[k].highlights;
              document.getElementById('selectedWeekLowlights').innerHTML=this.tableArray[k].lowlights;
              document.getElementById('selectedWeekCustEFT').innerHTML=this.tableArray[k].customer_eft;
              let highlightHeight=document.getElementById('selectedWeekHighlights').parentElement.parentElement.offsetHeight;
              let lowlightHeight=document.getElementById('selectedWeekLowlights').parentElement.parentElement.offsetHeight;
              let maxHeight=highlightHeight>lowlightHeight?highlightHeight:lowlightHeight;
              document.getElementById('selectedWeekHighlights').parentElement.parentElement.style.height=maxHeight+"px";
              document.getElementById('selectedWeekLowlights').parentElement.parentElement.style.height=maxHeight+"px";
            }
          }
        },
          loadTriggerMatrix(){
...
/** code for weekly trigger matrix */
      ...
      downloadCSVFile(excelData, filename) {
          ...
          csv_file = new Blob([excelData], {type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,"});
      ...
      for(let k=0;k<jsonData.length;k++){
        for(let l=0;l<keys.length;l++){
          jsonData[k][keys[l]]=jsonData[k][keys[l]].replaceAll('<a href="#" class="">','');
          jsonData[k][keys[l]]=jsonData[k][keys[l]].replaceAll('<a href="#" class="activeDate1">','');
...
<div class="wrapper row">
         ...
         <div class="percCountBtn row" style="position:relative;margin-bottom: 10px;">
          <button
          v-if="!showCount"
         ...
         <div class="row" style="margin-top:50px;">
          <div class="execSummaryTable">
                ...
                <tr ><td class="bg-primary tableHeading1"   colspan="100%"> 
                <span style="align:center"> FCS Readiness </span>
                <span
                 class="icon-download icon-small interaction"
                 style="float:right"
                 v-on:click="downloadTable('fcsRedinessTable')"
                 ></span>
                </td></tr>
                <tr>
                <td colspan="100%"> 
                 <table id="fcsRedinessTable" style="width:100%"> 
                 <tr>
                    <th class="uTableTh" style="width:20%">Customer CVT</th>
                    <th class="uTableTh" style="width:10%">Status</th>
                    <th class="uTableTh" style="width:70%">Details</th>
                </tr>
                <template v-for="(fObj,indx) in fcsRDetails">
                <tr :key="indx">
                     <td class="uTableTd" style="width:20%">{{fObj.cvtType}}</td>
                     <td style="width:10%" :class="'uTableTd '+checkStatusColor(fObj.status)" >{{fObj.status}}</td>
                     <td style="width:70%" class="uTableTd">
                     <template v-for="(dObj,inx) in fObj.details"><div :key="'details_'+indx+'_'+inx">{{dObj}}</div></template></td>
                </tr>
                 </template>
                    </table>
                </td>
        ...
        ></span>
              ...
              <th class="uTableTh">Pending Enable <br/>(total-{{pending_enablement_total}} )</th>
              <th class="uTableTh">CVT Raised</th>
               ...
               <tr :key='ik' :class="addActiveClass1(ik+1)">
                 <td  class="uTableTd">{{ik+1}}</td>
                  <td  class="uTableTd"><a :class="addActiveClass(ik+1)" href="#" @click="weekChange(ik+1)">{{tObj.weekDates}}</a></td>
                   <td  class="uTableTd">{{tObj.software_version}}</td>
                    <td v-if="!showCount" class="uTableTd">{{tObj.plannedCount}}</td>
                    <td v-if="!showCount" class="uTableTd">{{tObj.executedCount}}</td>
                    <td v-if="!showCount" class="uTableTd">{{tObj.pendingCount}}</td>
                     <td v-if="!showCount" class="uTableTd">{{tObj.passedCount}}</td>
                      <td v-if="!showCount" class="uTableTd">{{tObj.failedCount}}</td>
                   <td v-if="showCount" class="uTableTd">{{tObj.planned}}<span v-if="tObj.planned">%</span></td>
                    <td v-if="showCount" class="uTableTd">{{tObj.executed}}<span v-if="tObj.planned">%</span></td>
                     <td v-if="showCount" class="uTableTd">{{tObj.pending}}<span v-if="tObj.planned">%</span></td>
                     <td v-if="showCount" class="uTableTd">{{tObj.passed}}<span v-if="tObj.planned">%</span></td>
                      <td v-if="showCount" class="uTableTd">{{tObj.failed}}<span v-if="tObj.planned">%</span></td>
                       <td  class="uTableTd">{{tObj.quality}}</td>
                        <td  class="uTableTd">{{tObj.projectedQuality}}<span v-if="tObj.planned">%</span></td>
               ...
               <tr>
               <td class="bg-secondary" colspan="7" style="border: 1px solid #ddd;background: #b9e4b6;font-weight: 700;border:1px solid #a3bca2;padding: 5px !important;" >Highlights</td>
               <td class="bg-secondary" colspan="7" style="border: 1px solid #ddd;background: #f2d3c4;font-weight: 700;border: 1px solid #9a877e;padding: 5px !important;" >Lowlights</td>
               </tr>
               <tr>
               <td class="uTableTd" colspan="7">
    ...
    <div class="relWeekContentWrapper">
      ...
      <div id="selectedWeekLowlights"></div>
               ...
               <tr  v-if="!showMh"><td class="bg-primary tableHeading1"  colspan="100%"> 
               <span style="align:center"> Defects - {{defectCount}} </span>
               </td></tr>
                 <tr  v-if="showMh"><td class="bg-primary tableHeading1"  colspan="100%"> 
               <span style="align:center"> Must Have Defects - {{mhCount}} </span>
               </td></tr>
                <tr>
               <td colspan="16" class="uTableTd">
               <div class="cdetsToggleButton" style="position: relative;
    margin-right: 25px;">
               <button
                    v-if="!showMh"
                    class="btn btn--primary cm_addBtn1"
                ...
                <div class="issuesTable" v-if="showMh">
                 <tabs
                    id="cdetsTabs"
                    name="cdetsTabs"
                    :options="{ useUrlFragment: false, defaultTabHash:'TotalMHDefects' }"
              ...
              <tab  
               ...
               <th class="uTableTh issuesTh" style="width:45%;">Headline</th>
               ...
               <tr :key="'total_mh'+'___'+ic">
                    ...
                    <td class="uTableTd" style="width:45%;text-align: left;">{{cObj.headline}}</td>
                ...
                <template v-for="(cdetsObj,ic) in Object.keys(cdetsCommonArray)">
                <tab :key="'mh'+cdetsObj+'_'+ic"  
                 :name="cdetsObj"
                :id="'mh'+cdetsObj"
                :ref="'mh'+cdetsObj">
                <table id="issuesTable">
               ...
               <template v-for="(cObj,ic) in cdetsArray[cdetsObj]">
               <tr :key="cdetsObj+'___'+ic">
                ...
                <div class="issuesTable" v-if="!showMh">
                 <tabs
                    id="cdets1Tabs"
                    name="cdets1Tabs"
                    :options="{ useUrlFragment: false, defaultTabHash:'df_Total' }"
                    @clicked="cdetsTabClicked"
                    @changed="cdetsTabClicked">
               ...
               <template v-for="(cObj,ic) in mainCdetsArray[cdetsObj]">
               <tr :key="cdetsObj+'___'+ic">
                    <td class="uTableTd">{{cObj.defect_id}}</td>
                    <td class="uTableTd" style="width:45%;text-align: left;">{{cObj.headline}}</td>
                    <td class="uTableTd">{{cObj.product}}</td>
               ...
               <tr><td class="uTableTd" colspan="18">
                 <div class="relWeekContentWrapper">
                 <div class="summaryContentForWeek">
        ...
        ></span>
          ...
          <tr><td class="uTableTd" colspan="18">
           <table id="execRelTable" class="table table--highlight secondaryTable">
                ...
                <th class="rTableTh "> <span class="dot executedDot"></span>Executed</th>
                  <th class="rTableTh "> <span class="dot passedDot"></span>Quality</th>
             ...
             <tr class="custInfo">
             <th class="rTableTd folderTd"><span class="foldericon"><i class="icon icon-folder"></i></span><span class="projName">{{execCustomer.name}}</span></th>
              <td class="rTableTd ">{{execCustomer.Total.count}}</td>
             <td class="rTableTd ">{{execCustomer.executed.count}}({{execCustomer.executed.percentage}}%)</td>
              <td class="rTableTd ">{{execCustomer.quality}}%</td>
              <td class="rTableTd ">{{execCustomer.pending.count}}({{execCustomer.pending.percentage}}%)</td>
                   <td class="rTableTd ">{{execCustomer.passed.count}}({{execCustomer.passed.percentage}}%)</td>
                    <td class="rTableTd ">{{execCustomer.passx.count}}({{execCustomer.passx.percentage}}%)</td>
                     <td class="rTableTd ">{{execCustomer.failed.count}}({{execCustomer.failed.percentage}}%)</td>
                      <td class="rTableTd ">{{execCustomer.dropped.count}}({{execCustomer.dropped.percentage}}%)</td>
                       <td class="rTableTd ">{{execCustomer.blocked.count}}({{execCustomer.blocked.percentage}}%)</td>
                     
             </tr>
             <template v-for="(sobj,ipf) in execProjects[execCustomer.name]">
              <tr class="projectInfo" :key="ipf+'_aft'">
              <th class="rTableTd folderTd"><span class="foldericon" style="margin-left:10px;"><i class="icon icon-folder"></i></span><span class="projName">{{sobj.name}}</span></th>
                <td class="rTableTd ">{{sobj.total.count}}</td>
               <td class="rTableTd ">{{sobj.executed.count}} ({{sobj.executed.percentage}}%)</td>
                  <td class="rTableTd ">{{sobj.quality}}%</td>
                  <td class="rTableTd ">{{sobj.pending.count}} ({{sobj.pending.percentage}}%)</td>
                   <td class="rTableTd ">{{sobj.passed.count}} ({{sobj.passed.percentage}}%)</td>
                    <td class="rTableTd ">{{sobj.passx.count}} ({{sobj.passx.percentage}}%)</td>
                     <td class="rTableTd ">{{sobj.failed.count}} ({{sobj.failed.percentage}}%)</td>
                      <td class="rTableTd ">{{sobj.dropped.count}} ({{sobj.dropped.percentage}}%)</td>
                      <td class="rTableTd ">{{sobj.blocked.count}} ({{sobj.blocked.percentage}}%)</td>
                    
             </tr>
             <template v-for="(pobj,ip) in execFolders[sobj.name]">
              <tr :key="ip+'_abt'">
              <th class="rTableTd folderTd"><span class="foldericon" style="margin-left:20px;"><i class="icon icon-folder"></i></span><span class="projName">{{pobj.name}}</span></th>
               <td class="rTableTd ">{{pobj.total}}</td>
               <td class="rTableTd ">{{pobj.executed.count}} ({{pobj.executed.percentage}}%)</td>
                <td class="rTableTd ">{{pobj.quality}}%</td>
                 <td class="rTableTd ">{{pobj.pending.count}} ({{pobj.pending.percentage}}%)</td>
                  <td class="rTableTd ">{{pobj.passed.count}} ({{pobj.passed.percentage}}%)</td>
                    <td class="rTableTd ">{{pobj.passx.count}} ({{pobj.passx.percentage}}%)</td>
                     <td class="rTableTd ">{{pobj.failed.count}} ({{pobj.failed.percentage}}%)</td>
                     <td class="rTableTd ">{{pobj.dropped.count}} ({{pobj.dropped.percentage}}%)</td>
                      <td class="rTableTd ">{{pobj.blocked.count}} ({{pobj.blocked.percentage}}%)</td>
   ...
   <div class="tGradeMatrixTable" style="width:100%">
      <table id="tUpgradeTable" class="table table--highlight secondaryTable" style="width:100%">
      <tr>
     <th class="uTableTh" style="width:10%">Platform
      </th>
      <th class="uTableTh" style="width:10%">PID
   ...
   <div class="defectsHeatMap" style="width:100%">
      <chart  disbleExport=false  v-on:child="onChildInteraction" id="heatChart4" :option="upgradeHeatMapOptions"></Chart>
   </div>
  </div>
            </td>
            </tr>
