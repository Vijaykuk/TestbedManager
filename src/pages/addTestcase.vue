    ...
    formValues: {
      ...
      handler(newVal) {
        ...
        if (this.showingUnique || this.showingMaster) {
          let testcaseDetails = newVal.testcase_details;
          let uniqueRels = {};

          Object.entries(newVal.testcase_details).forEach(([key, value]) => {
            if (Object.keys(uniqueRels).indexOf(value.Release_version) <= -1) {
              uniqueRels[value.Release_version] = {};
              if (
                Object.keys(uniqueRels[value.Release_version]).indexOf(
                  value.Project_Name
                ) <= -1
              ) {
                uniqueRels[value.Release_version][value.Project_Name] = {};
                uniqueRels[value.Release_version][value.Project_Name][
                  value.folder_name + "_" + value.Tims_id
                ] = value.Tims_id;
              } else {
                uniqueRels[value.Release_version][value.Project_Name][
                  value.folder_name + "_" + value.Tims_id
  ...
  methods: {
    ...
    checklistChange() {
      ...
      if (event.currentTarget.value === "on") {
        for (let i = 0; i < temp.length; i++) {
          if (
            temp[i] != event.currentTarget.name &&
            (this.passCheckList[temp[i]] === false ||
              this.passCheckList[temp[i]] === "false")
          ) {
            return;
    ...
    timsForUniqueClicked(event) {
      // let timId=event.currentTarget.value;
      /* if(event.currentTarget.checked){
        if(this.uniqueSelectedTims.indexOf(timId)<0){
          this.uniqueSelectedTims.push(timId);
        }
      }
      else{
        if(this.uniqueSelectedTims.indexOf(timId)>-1){
          this.uniqueSelectedTims.splice(this.uniqueSelectedPrs.indexOf(timId),1);
        }
       }*/
    },

    prForUniqueClicked(event) {
      debugger;
      let tempTims = event.currentTarget.parentElement.getElementsByTagName(
        "input"
      );
      for (let m = 0; m < tempTims.length; m++) {
        if (event.currentTarget.checked) {
          tempTims[m].checked = true;
        } else {
          tempTims[m].checked = false;
        }
      }
    },
    timclicked(event) {},
    prExpandClicked(event) {
      let pr = event.currentTarget.innerText;
      let expandedPrs = { ...this.expandedPrs };

      if (Object.keys(expandedPrs).indexOf(pr) > -1) {
        expandedPrs[pr] = !expandedPrs[pr];
      } else {
        expandedPrs[pr] = true;
      }
      this.expandedPrs = { ...expandedPrs };
    },
    relExpandClicked(event) {
      let rel = event.currentTarget.innerText;
      let expandedRels = { ...this.expandedRels };

      if (Object.keys(expandedRels).indexOf(rel) > -1) {
        expandedRels[rel] = !expandedRels[rel];
      } else {
        expandedRels[rel] = true;
      }
      this.expandedRels = { ...expandedRels };
    },
    relForUniqueClicked(event) {
      let tempPrs = event.currentTarget.parentElement.getElementsByClassName(
        "projectsOf_" + event.currentTarget.value
      );
      for (let k = 0; k < tempPrs.length; k++) {
        let tempTims = tempPrs[k].getElementsByTagName("input");
        for (let m = 0; m < tempTims.length; m++) {
          if (event.currentTarget.checked) {
            tempTims[m].checked = true;
          } else {
    ...
    validateTestcaseForm() {
      if (!this.formValues.tc_title || !this.formValues.tc_feature) {
        if (
          this.formValues.tc_title &&
          (this.showingUnique  ||
          this.showingMaster) &&
          !this.formValues.tc_feature
        ) {
          return true;
        } else {
          return false;
    ...
    handleSubmit(e){
      ...
      if (this.validateTestcaseForm()) {
        ...
        json_obj["automation_flag"] =
          this.formValues.automation_flag === "true" ||
          this.formValues.automation_flag === true
            ? true
            : false;
        ...
        if (this.editForm === true) {
          ...
          if (this.showingUnique === true || this.showingMaster === true) {
            let uniqueSelectedTims = [];
            let uniqueTims1 = document.querySelectorAll(".selectedUniqueTims");
            for (let u = 0; u < uniqueTims1.length; u++) {
              if (uniqueTims1[u].checked) {
                uniqueSelectedTims.push(uniqueTims1[u].value);
              }
            }
            json_obj["testcase_details"] = uniqueSelectedTims;
          }
        }
        ...
        if (this.editForm === true) {
          if (this.showingUnique === false && this.showingMaster === false) {
            this.loading = true;
          ...
          } else if (this.showingUnique === true || this.showingMaster === true) {
...
<template>
  <div :class="testcaseRefId">
    <div class="tm_testCase_Form addForm2">
      <form class="testCaseForm">
        ...
        >
          ...
          >
            ...
            <!-- <div class="form-group__text tm-form__text">
              ...
              <label class="tm-label"> UUId : {{ formValues.tc_uuid }} </label>
            </div> -->
          </div>
        </div>
        <!-- v-show="(showingUnique || showingMaster) && !editForm" -->
        <div v-show="(!(showingUnique || showingMaster)) && editForm" class="form-group tm-form-group1 row" style="width:95%">
          <div style="width:30%">
            <div class="form-group__text tm-form__text">
               <label class="tm-label">
                Tims Id : {{ formValues.tims_id }}
        ...
        <div style="width:30%" class="form-group__text tm-form__text">
          <label class="tm-label">
                 UUId : {{ formValues.tc_uuid }} </label>
        ...
        <div class="form-group tm-form-group1 row" style="width:95%">
          ...
          <div style="width:20%" v-if="!showingUnique && !showingMaster">
            <div class="form-group__text tm-form__text">
        ...
        <div class="form-group tm-form-group" v-if="!showingUnique && !showingMaster">
          <div class="form-group__text tm-form__text">
            <input
              class="logFileClass"
              v-if="uploadImage"
            ...
            <label class="tm-label">
              ...
              <button
                class="btn btn--primary tm_addBtn logDeleteBtn"
                v-if="showLogDeleteButton"
                href="#"
              ...
              >
                <a
                  ...
                  @click="
                    showLogFiles(
                      formValues.id,
                      getFilename(formValues.tc_logFileName)[ind]
                    )
                  "
                  @contextmenu="
                    openLogMenu($event, {
        ...
        <div class="form-group tm-form-group" v-if="!showingUnique && !showingMaster">
          <div class="form-group__text tm-form__text">
            ...
            >
              <option
                v-for="(def, index) in assumedDefects"
                :key="index"
                style="margin-top: 5px;font-size: 1.5rem;"
                >{{ def.id }}: {{ def.desc }}</option
              >
            </select>
        ...
        <div class="form-group tm-form-group" v-if="showingUnique || showingMaster">
          <div class="form-group tm-form-group1 row" style="width:95%">
            <div class="" style="margin-top:20px">
              <label class="tm-label"> Select Tim: </label>
              <div class="chkboxwrapper row" style="margin-top:20px">
                <template v-for="(value, key) in uniqueRels">
                  <div class="chkRel" :key="'relName_' + key">
                    <div class="relClass">
                      <input
                        type="checkbox"
                        :value="key"
                        class="form-check-input tchk"
                        @click="relForUniqueClicked"
                      />
                      <a @click="relExpandClicked"
                        ><label class="form-check-label">{{ key }}</label></a
                      >
                      <div v-show="expandedRels[key]">
                        <template v-for="(prvalue, prkey) in value">
                          <div :key="'pr_' + prkey">
                            <div :class="'prClass projectsOf_' + key">
                              <input
                                type="checkbox"
                                :value="prkey"
                                class="form-check-input tchk"
                                @click="prForUniqueClicked"
                              />
                              <a @click="prExpandClicked"
                                ><label class="form-check-label">{{
                                  prkey
                                }}</label></a
                              >
                              <div v-show="expandedPrs[prkey]">
                                <template v-for="(timVal, timkey) in prvalue">
                                  <div
                                    :class="'timsClass timsOf_' + prkey"
                                    :key="
                                      'pr_' + key + '_' + prkey + '_' + timVal
                                    "
                                  >
                                    <input
                                      type="checkbox"
                                      :value="timVal"
                                      class="form-check-input tchk selectedUniqueTims"
                                      @click="timsForUniqueClicked"
                                    />
                                    <a @click="timclicked">
                                      <label class="form-check-label">{{
                                        timkey
        ...
        <div v-if="editForm && !showingUnique && !showingMaster" class="form-group tm-form-group">
          <div class="">
            <input
